...
upset(fromList(DMS_BP_G2_CparvsTpar), order.by = "freq", nsets = 8)
upset(fromList(DMS_BP_G2_CparvsTpar), order.by = "freq", nsets = 8, decreasing = F)

# Some annotation of the DMS shared by the maximum number of groups
library(data.table)

## Get DMS present in at least n BP
getSharedDMS <- function(mylist, n){
  setDT(stack(setNames(mylist, seq_along(mylist))))[,
                                                    if(uniqueN(ind)==n) values , values]$values
}
